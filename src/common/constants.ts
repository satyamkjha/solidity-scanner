export const SIDEBAR_WIDTH_EXPANDED = "270px";
export const SIDEBAR_WIDTH_COLLAPSED = "80px";

export const SLACK_CLIENT_ID = process.env.REACT_APP_SLACK_CLIENT_ID || "";
export const SLACK_SCOPE = "incoming-webhook";

export const BITBUCKET_CLIENT_ID =
  process.env.REACT_APP_BITBUCKET_CLIENT_ID || "";

export const GITLAB_CLIENT_ID = process.env.REACT_APP_GITLAB_CLIENT_ID || "";
export const GITLAB_SCOPES = process.env.REACT_APP_GITLAB_SCOPES || "";

export const JIRA_CLIENT_ID = "KEGs48Hu8dYlXw0qMbFf6VsEF6puMl3x";
export const JIRA_SCOPE =
  "read:jira-user write:jira-work read:jira-work offline_access";
