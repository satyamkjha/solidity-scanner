...
import { SeverityIcon, LogoIcon } from "components/icons";

import { useBetaScan } from "hooks/useScan";
import { useFileContent } from "hooks/useFileContent";
import { useIssueDetail } from "hooks/useIssueDetail";

import { ScanDetail } from "common/types";
...
const AdvancedScan: React.FC<{ scanId: string }> = ({ scanId }) => {
  const { data, refetch } = useBetaScan(scanId);
  const [file, setFile] = useState<FileState | null>(null);

  useEffect(() => {
    let intervalId: NodeJS.Timeout;
    const refetchTillScanComplete = () => {
      if (data && data.beta_scan_status !== "scan_done") {
  ...
  return (
    <>
      ...
      {data && (
        <>
          {data.beta_scan_status !== "scan_done" ? (
            <Flex
              w="100%"
              h="60vh"
              alignItems="center"
            ...
            <Flex w="100%" sx={{ flexDir: ["column", "column", "row"] }} py={2}>
              ...
              >
                <Box w="100%" minH="50vh">
                  <Issues issues={data.issues} file={file} setFile={setFile} />
                </Box>
              </VStack>
              <VStack
                w={["100%", "100%", "60%"]}
              ...
              >
                ...
                >
                  {file ? (
                    <>
                      <FileDetails file={file} type="project" />
                    </>
                  ) : (
                    <Flex
                      sx={{
    ...
    <Box w="100%">
      ...
      >
        <Box fontSize="sm" mb={2}>
          <IssueDetail issue_id={issue_id} description={file.description} />
        </Box>
      </Box>
    </Box>
  );
...
const IssueDetail: React.FC<{ issue_id: string; description: string }> = ({
  issue_id,
  description,
}) => {
